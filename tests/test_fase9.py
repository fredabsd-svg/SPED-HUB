...
class TestGraphQLSchema:
    ...
    def test_query_empresa_por_id(self, db):
        from src.api.graphql import schema
        result = schema.execute_sync(
            "{ empresas(pagina: 1, limite: 1) { dados { id } } }"
        )
        empresa_id = result.data["empresas"]["dados"][0]["id"]
        result = schema.execute_sync(
            f'{{ empresa(id: {empresa_id}) {{ id cnpj nome }} }}'
        )
        assert result.errors is None
    ...
    def test_query_ecd_por_id(self, db):
        from src.api.graphql import schema
        result = schema.execute_sync(
            "{ ecds(pagina: 1, limite: 1) { dados { id } } }"
        )
        ecd_id = result.data["ecds"]["dados"][0]["id"]
        result = schema.execute_sync(
            f'{{ ecd(id: {ecd_id}) {{ id empresaNome leiaute }} }}'
        )
        assert result.errors is None
    ...
    def test_query_dfc(self, db):
        from src.api.graphql import schema
        result = schema.execute_sync(
            f'{{ dfc(ecdId: {db}) {{ titulo variacaoCaixa }} }}'
        )
        assert result.errors is None
    ...
    def test_query_notas(self, db):
        from src.api.graphql import schema
        result = schema.execute_sync(
            f'{{ notas(ecdId: {db}) {{ numero titulo tipo }} }}'
        )
        assert result.errors is None
    ...
    def test_query_evolucao_multi(self, db):
        from src.api.graphql import schema
        result = schema.execute_sync(
            f'{{ evolucaoMulti(ecdId: {db}) {{ numPeriodos }} }}'
        )
        assert result.errors is None
...
class TestParserStreaming:
    ...
    def test_parse_streaming_mantem_resultados(self):
        ...
        for t, s in zip(todos, streaming):
...
class TestExportMultiFormato:
    ...
    def test_export_xlsx_to_buffer(self, db):
        ...
        try:
            ...
            export = ExportEngine()
            buffer = io.BytesIO()
            export.export_xlsx_to_buffer(buffer, ctx, linhas_dict, ["secao", "cod_cta", "nome_cta", "saldo_atual"], ctx.titulo)
            buffer.seek(0)
    ...
    def test_export_zip_multi_formato(self, db):
        ...
        try:
            ...
            with zipfile.ZipFile(zip_buffer, "w", zipfile.ZIP_DEFLATED) as zf:
                ...
                export.export_xlsx_to_buffer(xlsx_buffer, ctx, linhas_dict, ["secao", "cod_cta", "nome_cta", "saldo_atual"], ctx.titulo)
