...
| `WebhookService.retry_failed(webhook_id=None)` | Reenvio **manual**: lote de `failed` **e** abandonadas; devolve `restantes`. |
| `WebhookService.reenviar_abandonadas(webhook_id=None)` | Reenvio **automático**: só o que a queda interrompeu. |
...
- **O backoff roda com `asyncio.sleep` dentro do próprio `dispatch`** —
  ...
  por tentativa).
- **Automático e manual têm escopos diferentes, e isso é o ponto.**
  `reenviar_abandonadas` (chamado pelo laço de manutenção) só toca entrega cujo
  orçamento de tentativas nunca foi gasto — o processo morreu antes, e o
  assinante pode estar perfeitamente no ar. `retry_failed` (o botão do painel)
  inclui também as `failed`, que responderam mal em **todas** as tentativas:
  reenviar essas sozinho, de hora em hora, marteliza um endereço quebrado sem
  resolver nada. Sem essa separação, automatizar o reenvio seria pior que não
  ter. `SPED_HUB_WEBHOOK_AUTO_RETRY=false` desliga o automático.
...
- Não garante entrega: sem assinante o evento é descartado, e falha após
  ...
  abandonadas.
- Não reenvia sozinho o que esgotou as tentativas: só o que uma queda
  interrompeu. `failed` espera o botão do painel, por escolha.
