...
| 46 | `sped-hub fiscal`: a cadeia da Central pela linha de comando | concluída | `tests/test_cli_fiscal.py` | importar, listar, classificar, alterar, desfazer, gerar e conferir. `classificar` e `alterar` **não gravam** sem que se peça; gerar **sempre** arquiva; `conferir` sai com 2 quando o entregue divergiu. Cadastro de regras e telas web seguem fora |
