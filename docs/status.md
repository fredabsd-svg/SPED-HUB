...
| 73 | O `0200` da EFD-Contribuições sai com o leiaute dela, não com o da EFD ICMS/IPI | concluída | `tests/test_portas_de_entrada.py::TestRegistro0200DaEFDContribuicoesPelaCLI`, `tests/test_leiaute_sped.py::TestOsDoisLeiautesDoMesmoRegistro` | o registro estava em `_COMUNS` com o `CEST` do ICMS, e **o 0200 da EFD-Contribuições saía com um campo a mais** — recusado pelo validador. A palavra CEST não aparece nas 433 páginas do Guia Prático 1.35. `conferir` não pega isto: ele compara o gerador com a nossa tabela, e era a tabela que estava errada |
